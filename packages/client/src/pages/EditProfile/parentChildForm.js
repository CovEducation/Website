...
const ParentChildForm = (props) => {
  ...
  const subjectsMenuItems = subjects.map((item, index) => {
    var isChecked = false;
    if (props.data.registeredChildren[props.index].subjects !== undefined) {
      isChecked =
        props.data.registeredChildren[props.index].subjects.indexOf(
          item.value
        ) > -1;
    }
  ...
  const handleChange = (event) => {
    ...
    props.updateRegisteredChild(props.index, {
      ...state,
