...
const MentorStep2 = (props) => {
  ...
  const gradeLevelMenuItems = tags.map((item) => (
    <MenuItem key={item.value} value={item.value}>
      <Checkbox
        value={item.value}
        checked={selectedGradeLevels.indexOf(item.value) > -1}
      />
      {item.label}
    </MenuItem>
  ));
  ...
  const proNounsMenuItems = proNouns.map((item) => {
    return (
      <MenuItem key={item.value} value={item.pronoun}>
        {item.pronoun}
      </MenuItem>
    );
  });
  ...
  return (
    <div>
      ...
      <WizardInput>
        <InputLabel id="wizard-preferred-grade-level">
      ...
      <WizardInput>
        <InputLabel id="wizard-notification-preference">
